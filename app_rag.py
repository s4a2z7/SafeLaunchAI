"""
SafeLaunch AI - RAG 기반 완전 통합 웹서비스
프론트엔드(Streamlit) + 백엔드(RAG Engine) 통합 버전

기존 startup-legal-helper 프로젝트의 RAG 엔진을 활용하여
실제 법률 데이터베이스와 연동된 서비스 구현
"""
...
def analyze_service_risk(
    ...
    try:
        ...
        # 리스크 점수 계산
        risk_score = calculate_risk_score(search_results, service_description)
        ...
        analysis = {
            ...
            "recommendations": generate_recommendations(search_results, risk_score)
        }
    ...
    with st.spinner("🔄 RAG 시스템 초기화 중..."):
        ...
        if stats:
            ...
            with col3:
                st.markdown(f"""
                <div class="stat-box">
                    <div class="stat-number">{stats.get('policies', 0):,}</div>
