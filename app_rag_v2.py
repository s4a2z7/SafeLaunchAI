"""
SafeLaunch AI - RAG 기반 완전 통합 웹서비스 (V2 - HTML 렌더링 수정)
프론트엔드(Streamlit) + 백엔드(RAG Engine) 통합 버전
"""
...
if st.session_state.analysis_result:
    ...
    # 초기 화면 - Streamlit 컴포넌트만 사용
    st.markdown("### 👋 SafeLaunch AI RAG Edition에 오신 것을 환영합니다!")
    
    # HTML 대신 Streamlit 네이티브 컴포넌트 사용
    with st.container():
        st.markdown("#### 🎯 이 서비스는 무엇을 하나요?")
        st.write("실제 법률 데이터베이스를 기반으로 귀하의 서비스가 가진 법적 리스크를 분석합니다.")
        
        st.markdown("#### 📚 데이터 소스")
        st.markdown("""
        - **법률 조항**: 저작권법, 개인정보보호법, 정보통신망법 등
        - **판례**: 과거 IT 서비스 관련 분쟁 사례
        - **스토어 정책**: Google Play, App Store 가이드라인
        """)
        
        st.markdown("#### 🔍 분석 방법")
        st.markdown("""
        1. **벡터 검색**: TF-IDF 기반 유사도 계산
        2. **리스크 평가**: 검색 결과 기반 점수 산정
        3. **권장사항 생성**: 실행 가능한 조치 제시
        """)
